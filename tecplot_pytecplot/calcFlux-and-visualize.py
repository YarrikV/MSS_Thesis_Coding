...
for run_name in tqdm(ALL_FOLDERS):
    ...
    tp.data.operate.execute_equation(equation=f'{{bz_above_or_below}}=IF({{bz_eq}}>{B_CLT_THRESHOLD},1,IF({{bz_eq}}<-{B_CLT_THRESHOLD},1,0))',
                                     ignore_divide_by_zero=True)
    ...
    # blanking of bz_eq
    run.blanking(1, "bz_eq", RelOp.LessThan, B_CLT_THRESHOLD, active=False)
    run.blanking(2, "bz_eq", RelOp.GreaterThan, -
                 B_CLT_THRESHOLD, active=False)

    # blanking for visuals
    run.blanking(3, "bz_above_or_below", RelOp.EqualTo, 0, active=False)
    ...
    if doPlus:
        ...
        tp.export.save_png(os.path.join(run.store_gfx_path, "flux",
    ...
    plot.contour(0).levels.reset_levels([-5, 5])
    ...
    logger.debug("Done exporting png.")
